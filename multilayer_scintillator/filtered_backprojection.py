...
def getTrueImg(mu_mapping, thickness, energy_tgt_ind, xraySrc):
    Nx, Nz, numEnergy = mu_mapping.shape
    TrueImg = np.zeros_like(mu_mapping)

    for nE in range(numEnergy):
        mu_temp = mu_mapping[:,:,nE]
        TrueImg[:,:,nE] = xraySrc[nE]*np.exp(-mu_temp*thickness)

    TrueImg_by_interval = np.zeros((Nx, Nz, energy_tgt_ind.size-1))
    for nE in range(energy_tgt_ind.size-1):
        TrueImg_by_interval[:,:,nE] = np.sum(TrueImg[:,:,energy_tgt_ind[nE]:energy_tgt_ind[nE+1]], axis=2)/(np.sum(xraySrc[energy_tgt_ind[nE]:energy_tgt_ind[nE+1]]))

    return TrueImg_by_interval

def getProj_phantom(mu_mapping, thickness, theta, P, energy_tgt_ind, xraySrc, noise):
    Nx, Nz, numEnergy = mu_mapping.shape
    numAngles = len(theta)
    sinogram = np.zeros((Nx, numAngles, numEnergy))

    #get projections and plot
    for nE in range(numEnergy):
        mu_temp = mu_mapping[:,:,nE]
        mu_min = np.min(mu_temp)
        mu_max = np.max(mu_temp)
        mu_temp = 255*(mu_temp - mu_min)/(mu_max - mu_min)
        
        img_temp = Image.fromarray(mu_temp)
        
        for nA in range(numAngles):
            rotImgObj = img_temp.rotate(90-theta[nA], resample=Image.BICUBIC)
            mu_rot = np.array(rotImgObj)*(mu_max - mu_min)/255 + mu_min
            
            sinogram[:,nA,:] += xraySrc[nE]*np.exp(-np.sum(mu_rot*thickness, axis=1))[:,np.newaxis]*P[nE,:][np.newaxis,:]

    sinogram_by_interval = np.zeros((Nx, numAngles, energy_tgt_ind.size-1))
    for nE in range(energy_tgt_ind.size-1):
        sinogram_by_interval[:,:,nE] = np.sum(sinogram[:,:,energy_tgt_ind[nE]:energy_tgt_ind[nE+1]], axis=2)/(np.sum(xraySrc[energy_tgt_ind[nE]:energy_tgt_ind[nE+1]]))

    sinogram_by_interval *= (1 + noise*np.random.normal(size=(Nx, numAngles, energy_tgt_ind.size-1)))

    return sinogram_by_interval
