...
#                     centroid=centroid, label=label, d_mean=d_mean, ind_d_mean=ind_d_mean, d_mean_list=self.d_mean_list)
    ...
    def reconstruct_image(self, source_energy_list, source_xy_list, xy_list, RGB_list, event_label, nCluster=None, nTrials=10):
        ...
        if self.RGB_scintillator:
            ...
            min_cluster_size = 50 # 100,200,200
            ...
            if xy_list.shape[0] > min_cluster_size:
                ...
                centroid, label = self.gmeans(np.hstack((xy_list, RGB_list)), 20, 20, min_cluster_size=min_cluster_size, max_cluster_size=max_cluster_size, RGB=True)
                nk = centroid.shape[0]
                ind_RGB = np.argmax(centroid[:,2:], axis=1)
        ...
        if RGB:
            ...
            for nC in range(centroid.shape[0]):
                if ind_merged[nC] == 0:
                    ...
                    if np.sum(dist < self.dimDet[0]/5e4) > 1:
                        index = np.argwhere(dist < self.dimDet[0]/5e4)[:,0]
                        
                        num = 0
                        centroid_temp = np.zeros(centroid.shape[1])
                        for i in index:
                            num += np.sum(label==i)
                            centroid_temp += np.sum(xy_list[label==i,:], axis=0)
                            label_merged[label==i] = nC - np.sum(ind_merged[:nC]) + n_merged
                            ind_merged[i] = 1
                        
                        n_merged += 1
                        
                        if centroid_merged is None:
                            centroid_merged = centroid_temp/num
                        else:
                            centroid_merged = np.vstack((centroid_merged, centroid_temp/num))
                        ...
                        label_merged[label==nC] = nC - np.sum(ind_merged[:nC]) + n_merged
        else:
...
#        print(i+1, flush=True)
    ...
    imaging_mode = False
    Nphoton_rate_list = np.hstack((np.linspace(10, 100, 10), np.linspace(200, 500, 4))).astype(int)
    #np.hstack((np.linspace(10, 100, 10), np.linspace(200, 500, 4))).astype(int) / np.linspace(10, 100, 10).astype(int)
    Nphoton_rate_list_plot = np.hstack((np.linspace(10, 100, 10), np.linspace(200, 500, 4))).astype(int)
    #np.hstack((np.linspace(10, 100, 10), np.linspace(200, 500, 4))).astype(int) / np.linspace(10, 100, 10).astype(int)
