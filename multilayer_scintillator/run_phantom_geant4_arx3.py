...
#                 fEventHits=np.array(hits["fEvent"].array()), fEventSrc=np.array(source["fEvent"].array()))
    ...
    def RGB_readings2(self, wvl_list):
        ...
        for i in range(wvl_list.size):
            sensRGB_interp = np.interp(wvl_list, self.sensRGB[:,0], self.sensRGB[:,i+1])
            prob = np.append(sensRGB_interp, 1-np.sum(sensRGB_interp))
            RGB[i] = np.random.choice(np.arange(4), p=prob)
...
#                     centroid=centroid, label=label, d_mean=d_mean, ind_d_mean=ind_d_mean, d_mean_list=self.d_mean_list)
    ...
    def reconstruct_image(self, source_energy_list, source_xy_list, xy_list, RGB_list, event_label, nCluster=None, nTrials=10):
        ...
        if self.RGB_scintillator:
            ...
            max_cluster_size = np.array([500,1000,500]) # 300,600,600
            ...
            if RGB_list2 is not None:
                ...
                nk = centroid_merged.shape[0]
                fm = fowlkes_mallows_index(event_label_filt2, label_merged)
                sc = silhouette_coeff(nk, xy_list2, np.ones(xy_list2.shape[0]), centroid=centroid_merged, label=label_merged)
                
                d_mean = np.zeros(nk)
                for k in range(nk):
                    npts = int(np.sum(label_merged==k))
                    if npts > 0:
                        xy_temp = xy_list2[label_merged==k,:]
                        ...
                        d_mean[k] = np.sum(np.sqrt(np.sum((np.tile(centroid_merged[k,:2], (npts_kde,1)) - xy_temp[kde>np.max(np.hstack((kde, kde_temp)))/10,:])**2, axis=1)))/npts_kde
                        ind_d_mean = np.argmin(np.abs(d_mean[k] - self.d_mean_list))
                        
                        while True:
                            xrayInc = energy_distribution_norm[int(ind_RGB[k]),ind_d_mean,:]
                        ...
                        ind_x0 = int((centroid_merged[k,0] - self.dimDetVoxel[0]/2e3)//(self.dimDetVoxel[0]/1e3))
                        x0 = self.dimDetVoxel[0]/2e3 + ind_x0*self.dimDetVoxel[0]/1e3
                        ind_x1 = ind_x0 + 1
                        x1 = self.dimDetVoxel[0]/2e3 + ind_x1*self.dimDetVoxel[0]/1e3
                            
                        ind_y0 = int((centroid_merged[k,1] - self.dimDetVoxel[1]/2e3)//(self.dimDetVoxel[1]/1e3))
                        y0 = self.dimDetVoxel[1]/2e3 + ind_y0*self.dimDetVoxel[1]/1e3
                        ind_y1 = ind_y0 + 1
                        y1 = self.dimDetVoxel[1]/2e3 + ind_y1*self.dimDetVoxel[1]/1e3
            
                        ratio = np.zeros(4)
                        ratio[0] = np.sqrt(np.sum((np.array([x0,y0]) - centroid_merged[k,:])**2))
                        ratio[1] = np.sqrt(np.sum((np.array([x0,y1]) - centroid_merged[k,:])**2))
                        ratio[2] = np.sqrt(np.sum((np.array([x1,y0]) - centroid_merged[k,:])**2))
                        ratio[3] = np.sqrt(np.sum((np.array([x1,y1]) - centroid_merged[k,:])**2))
                        if np.any(ratio == 0):
                    ...
                    if os.path.exists(directory + "/data/reconstruction_examples/" + self.identifier + "/clustering_pRate_" + str(nk) + "_min.npz"):
                        ...
                        if fm < data['fm']:
                            np.savez(directory + "/data/reconstruction_examples/" + self.identifier + "/clustering_pRate_" + str(nk) + "_min", d_mean=d_mean, xrayInc=xrayInc_all,
                                     ...
                                     centroid_merged=centroid_merged, label_merged=label_merged)
                    else:
                        np.savez(directory + "/data/reconstruction_examples/" + self.identifier + "/clustering_pRate_" + str(nk) + "_min", d_mean=d_mean, xrayInc=xrayInc_all,
                                 ind_x0=ind_x0, ind_x1=ind_x1, x0=x0, x1=x1, ind_y0=ind_y0, ind_y1=ind_y1, y0=y0, y1=y1, ratio=ratio, xy_list=xy_list, xy_list2=xy_list2, RGB_list=RGB_list,
                                 RGB_list2=RGB_list2, source_energy_list=source_energy_list, source_xy_list=source_xy_list, centroid0=centroid0, label0=label0, event_label=event_label,
...
#        print(i+1, flush=True)
    ...
    imaging_mode = False
    Nphoton_rate_list = np.linspace(10, 100, 10).astype(int)
    #np.hstack((np.linspace(10, 100, 10), np.linspace(200, 500, 4))).astype(int) / np.linspace(10, 100, 10).astype(int)
    Nphoton_rate_list_plot = np.linspace(10, 100, 10).astype(int)
    #np.hstack((np.linspace(10, 100, 10), np.linspace(200, 500, 4))).astype(int) / np.linspace(10, 100, 10).astype(int)
