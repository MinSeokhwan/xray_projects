...
class geant4:
    def __init__(self,
        ...
        if self.RGB_scintillator:
            self.d_mean_list = np.geomspace(1e-1, np.max(self.dimDet[:2])/5e3, d_mean_bins) # mm
        else:
            self.d_mean_list = np.geomspace(5e-2, np.max(self.dimDet[:2])/1e4, d_mean_bins) # mm
...
#        np.savez(directory + "/data/debug_read_output" + str(comm.rank + self.rank_start), wvl_list=wvl_list, eventHits=eventHits, xy_list=xy_list, energySrc_list=energySrc_list, xySrc_list=xySrc_list,
...
#                 fEventHits=np.array(hits["fEvent"].array()), fEventSrc=np.array(source["fEvent"].array()))
    ...
    def RGB_readings2(self, wvl_list):
        ...
        for i in range(3):
            ...
            #prob = np.append(sensRGB_interp, 1-np.sum(sensRGB_interp))
    ...
    def data_interpretation(self, xy_list, RGB_list=None):
        if self.RGB_scintillator:
            RGB_weights = np.zeros(3)
            d_mean = np.zeros(3)
            ind_d_mean = np.zeros(3)
            for i in range(3):
                if np.sum(np.argmax(RGB_list, axis=1)==i) > 10:
                    RGB_weights[i] = 1 #np.sum(np.argmax(RGB_list, axis=1)==i)
                    xy_i = xy_list[np.argmax(RGB_list, axis=1)==i,:]
                    
                    centroid, label = weighted_kmeans(xy_i, np.ones(xy_i.shape[0]), nCluster=1, maxiter=20, centroid=(self.dimDet[:2]/1e3).reshape(1,-1)) # weights-->RGB_list
                    
                    xy_temp = np.array(list(product(np.linspace(np.min(xy_i[:,0]), np.max(xy_i[:,0]), int(1e2)),
                                                    np.linspace(np.min(xy_i[:,1]), np.max(xy_i[:,1]), int(1e2)))))
                
                    try:
                        kernel = gaussian_kde(xy_i.T)
                        kde = kernel(xy_i.T)
                        kde_temp = kernel(xy_i.T)
                        
                        npts = np.sum(kde>np.max(np.hstack((kde, kde_temp)))/10)
                        d_mean[i] = np.sum(np.sqrt(np.sum((centroid[0,:][np.newaxis,:] - xy_i[kde>np.max(np.hstack((kde, kde_temp)))/10,:])**2, axis=1)))/npts
                        
                        ind_d_mean[i] = np.argmin(np.abs(d_mean[i] - self.d_mean_list))
                    except:
                        pass
            
#            if np.sum(RGB_weights) > 0:
#                RGB_weights /= np.sum(RGB_weights)
            
            return ind_d_mean, d_mean, RGB_weights
...
#                     centroid=centroid, label=label, d_mean=d_mean, ind_d_mean=ind_d_mean, d_mean_list=self.d_mean_list)
    ...
    def reconstruct_image(self, source_energy_list, source_xy_list, xy_list, RGB_list, event_label, nCluster=None, nTrials=10):
        ...
        if self.RGB_scintillator:
            ...
            xyR = np.argmax(RGB_list, axis=1) == 0
            xyG = np.argmax(RGB_list, axis=1) == 1
            xyB = np.argmax(RGB_list, axis=1) == 2
            for i in range(nCluster):
                if np.sum(event_label==i) > 0:
                    nPhotonR = np.append(nPhotonR, np.sum((event_label==i)*(xyR)))
                    nPhotonG = np.append(nPhotonG, np.sum((event_label==i)*(xyG)))
                    nPhotonB = np.append(nPhotonB, np.sum((event_label==i)*(xyB)))
            
            min_cluster_size = np.array([10,10,10]) # 100,200,200
            max_cluster_size = np.array([1000,1000,1000]) # 300,600,600
            ...
            for i in range(3):
                if np.sum(np.argmax(RGB_list, axis=1)==i) > min_cluster_size[i]:
                    ...
                    event_label_i = event_label[np.argmax(RGB_list, axis=1)==i]
            ...
            if RGB_list2 is not None:
                ...
                for i in range(3):
                    ...
                    if xy_i.shape[0] > min_cluster_size[i]:
                        centroid, label = self.gmeans(xy_i, 20, 20, min_cluster_size=min_cluster_size[i], max_cluster_size=max_cluster_size[i], color=i)
                ...
                if comm.rank == 0:
                    np.savez(directory + "/data/debug_gmeans_final_rank" + str(comm.rank + self.rank_start), centroid0=centroid0, label0=label0, xy_list=xy_list2, RGB_list=RGB_list2,
                             ind_RGB=ind_RGB)
                
#                RGB0 = np.zeros(centroid0.shape[0])
#                for nC in range(centroid0.shape[0]):
#                    RGB_nC = np.mean(RGB_list2[label0==i,:], axis=0)
#                    #RGB_transform = np.linalg.inv(self.RGB_basis) @ RGB_nC
#                    RGB0[nC] = np.argmax(np.abs(RGB_nC))
#                
#                centroid_merged = None
#                ind_RGB = np.zeros(0)
#                label_merged = np.zeros(xy_list2.shape[0])
#                ind_merged = np.zeros(centroid0.shape[0])
#                for nC in range(centroid0.shape[0]):
#                    if ind_merged[nC] == 0:
#                        dist = np.sqrt(np.sum((np.tile(centroid0[nC,:], (centroid0.shape[0],1)) - centroid0)**2, axis=1))
#                        if np.sum((dist < self.dimDet[0]/2.5e4)*(RGB0[nC]!=RGB0)) > 1:
#                            index = np.argwhere((dist < self.dimDet[0]/2.5e4)*(RGB0[nC]!=RGB0))[:,0]
#                                               
#                            mask_nC = np.isin(label0, index)
#
#                            for i in index:
#                                ind_merged[i] = 1
#                            
#                            if centroid_merged is None:
#                                centroid_merged = np.mean(xy_list2[mask_nC,:], axis=0)
#                                label_merged[mask_nC] = 0
#                            else:
#                                centroid_merged = np.vstack((centroid_merged, np.mean(xy_list2[mask_nC,:], axis=0)))
#                                label_merged[mask_nC] = np.max(label_merged) + 1
#                            ind_RGB = np.append(ind_RGB, np.argmax(np.sum(xy_list2[mask_nC,:], axis=0)))
#                        else:
#                            if centroid_merged is None:
#                                centroid_merged = centroid0[nC,:]
#                                label_merged[label0==nC] = 0
#                            else:
#                                centroid_merged = np.vstack((centroid_merged, centroid0[nC,:]))
#                                label_merged[label0==nC] = np.max(label_merged) + 1
#                            ind_RGB = np.append(ind_RGB, np.argmax(np.sum(xy_list2[label0==nC,:], axis=0)))
#            
#                if len(centroid_merged.shape) == 1:
#                    centroid_merged = centroid_merged.reshape(1, -1)
#                
#                if comm.rank == 0:
#                    np.savez(directory + "/data/debug_gmeans_merged_" + str(cnt) + "_rank" + str(comm.rank + self.rank_start), centroid=centroid_merged, label=label_merged,
#                             xy_list=xy_list2, RGB_list=RGB_list2)
                
                cluster_weights = np.zeros(centroid0.shape[0])
                for nC in range(centroid0.shape[0]):
                    dist = np.sqrt(np.sum((np.tile(centroid0[nC,:], (centroid0.shape[0],1)) - centroid0)**2, axis=1))
                    cluster_weights[nC] = 1/(1 + np.sum((dist < self.dimDet[0]/2.5e4)*(ind_RGB[nC]!=ind_RGB)))
                
                nk = centroid0.shape[0]
                fm = fowlkes_mallows_index(event_label_filt2, label0)
                sc = silhouette_coeff(nk, xy_list2, np.ones(xy_list2.shape[0]), centroid=centroid0, label=label0)
                
                d_mean = np.zeros(nk)
                for k in range(nk):
                    npts = int(np.sum(label0==k))
                    if npts > 0:
                        xy_temp = xy_list2[label0==k,:]
                        ...
                        npts_kde = np.sum(kde>np.max(np.hstack((kde, kde_temp)))/10)
                
                        d_mean[k] = np.sum(np.sqrt(np.sum((np.tile(centroid0[k,:2], (npts_kde,1)) - xy_temp[kde>np.max(np.hstack((kde, kde_temp)))/10,:])**2, axis=1)))/npts_kde
                        ind_d_mean = np.argmin(np.abs(d_mean[k] - self.d_mean_list))
                        
                        while True:
                            xrayInc = energy_distribution_norm[int(ind_RGB[k]),ind_d_mean,:]*cluster_weights[k]
                            # xrayInc = savgol_filter(xrayInc, 10, 5)
                            # xrayInc[xrayInc<0] = 0
                            if np.sum(xrayInc) > 0:
                                if source_energy_list.size == 1:
                                    for ntgt in range(self.energy_tgt.size-1):
                                        if source_energy_list[0] >= self.energy_tgt[ntgt] and source_energy_list[0] < self.energy_tgt[ntgt+1]:
                                            break
                                    Mconf[ntgt,:] += xrayInc
                        ...
                        ind_x0 = int((centroid0[k,0] - self.dimDetVoxel[0]/2e3)//(self.dimDetVoxel[0]/1e3))
                        x0 = self.dimDetVoxel[0]/2e3 + ind_x0*self.dimDetVoxel[0]/1e3
                        ind_x1 = ind_x0 + 1
                        x1 = self.dimDetVoxel[0]/2e3 + ind_x1*self.dimDetVoxel[0]/1e3
                            
                        ind_y0 = int((centroid0[k,1] - self.dimDetVoxel[1]/2e3)//(self.dimDetVoxel[1]/1e3))
                        y0 = self.dimDetVoxel[1]/2e3 + ind_y0*self.dimDetVoxel[1]/1e3
                        ind_y1 = ind_y0 + 1
                        y1 = self.dimDetVoxel[1]/2e3 + ind_y1*self.dimDetVoxel[1]/1e3
            
                        ratio = np.zeros(4)
                        ratio[0] = np.sqrt(np.sum((np.array([x0,y0]) - centroid0[k,:])**2))
                        ratio[1] = np.sqrt(np.sum((np.array([x0,y1]) - centroid0[k,:])**2))
                        ratio[2] = np.sqrt(np.sum((np.array([x1,y0]) - centroid0[k,:])**2))
                        ratio[3] = np.sqrt(np.sum((np.array([x1,y1]) - centroid0[k,:])**2))
                        if np.any(ratio == 0):
                    ...
                    if os.path.exists(directory + "/data/reconstruction_examples/" + self.identifier + "/clustering_pRate_" + str(nk) + "_min.npz"):
                        ...
                        if fm < data['fm']:
                            np.savez(directory + "/data/reconstruction_examples/" + self.identifier + "/clustering_pRate_" + str(nk) + "_min", d_mean=d_mean, xrayInc=xrayInc_all,
                                     ...
                                     sc=sc, ind_RGB=ind_RGB, fm=fm, energy_distribution_norm=energy_distribution_norm,
                                     event_label_filt2=event_label_filt2, sc_ground_truth=sc_ground_truth, centroid=centroid, label=label, xy_i=xy_i)
                    else:
                        np.savez(directory + "/data/reconstruction_examples/" + self.identifier + "/clustering_pRate_" + str(nk) + "_min", d_mean=d_mean, xrayInc=xrayInc_all,
                                 ind_x0=ind_x0, ind_x1=ind_x1, x0=x0, x1=x1, ind_y0=ind_y0, ind_y1=ind_y1, y0=y0, y1=y1, ratio=ratio, xy_list=xy_list, xy_list2=xy_list2, RGB_list=RGB_list,
                                 RGB_list2=RGB_list2, source_energy_list=source_energy_list, source_xy_list=source_xy_list, centroid0=centroid0, label0=label0, event_label=event_label,
            ...
            return image_reconstr_temp, sc_ground_truth, sc, fm, nk, nPhotonR, nPhotonG, nPhotonB, Mconf
            ...
            if xy_list.shape[0] > 100:
                sc_ground_truth = silhouette_coeff(int(np.max(event_label)), xy_list, np.ones(xy_list.shape[0]), centroid=source_xy_list, label=event_label)
            
                centroid, label = self.gmeans(xy_list, 20, 20, min_cluster_size=100, max_cluster_size=1200)
                nk = centroid.shape[0]
                
                if not self.verbose:
                    with open(directory + "/logs/Rank" + str(comm.rank + self.rank_start) + ".txt", 'a') as log:
                        log.write('    Gmeans\n')
                ...
                for k in range(nk):
                    ...
                    if npts > 10:
#                        r_list = np.sqrt(np.sum((np.tile(centroid[k,:], (npts,1)) - xy_list[label==k,:])**2, axis=1))
#                        for i in range(self.d_mean_bins):
#                            npts_vs_r = np.sum(r_list < self.d_mean_list[i])
#                            if npts_vs_r > 0.9*npts:
#                                d_mean[k] = self.d_mean_list[i-1]
#                                ind_d_mean = i - 1
#                                break
            ...
            return image_reconstr_temp, sc_ground_truth, sc, fm, nk, nPhoton, Mconf
    
    def gmeans(self, xy_list, maxiter_gmeans, maxiter_kmeans, min_cluster_size=1000, max_cluster_size=1000, p_crit=0.05, color=0):
...
#                log.write('\nIteration ' + str(cnt) + '\n')
            ...
            if centroid.shape[0] > 1:
                ...
                for nC1 in range(centroid.shape[0]):
                    if not ind_merged[nC1]:
                        dist = np.sqrt(np.sum((centroid[nC1,:][np.newaxis,:] - centroid)**2, axis=1))
                        nC2 = np.argsort(dist)[1]
                        ...
                        if (pval > p_crit and np.sum(mask_nC) < max_cluster_size):
                            ind_merged[nC2] = 1
                            if centroid_merged is None:
                                centroid_merged = np.mean(xy_nC, axis=0)
                                label_merged[mask_nC] = 0
                            else:
                                centroid_merged = np.vstack((centroid_merged, np.mean(xy_nC, axis=0)))
                                label_merged[mask_nC] = np.max(label_merged) + 1
                        else:
                            if centroid_merged is None:
                                centroid_merged = centroid[nC1,:].reshape(1, -1)
                                label_merged[label==nC1] = 0
                            else:
                                centroid_merged = np.vstack((centroid_merged, centroid[nC1,:]))
                                label_merged[label==nC1] = np.max(label_merged) + 1
                
                if len(centroid_merged.shape) == 1:
                    centroid_merged = centroid_merged.reshape(1, -1)
            
                centroid, label = weighted_kmeans(xy_list, np.ones(xy_list.shape[0]), centroid_merged.shape[0], maxiter_kmeans, centroid=centroid_merged)
            
            fm = fowlkes_mallows_index(label_prev, label)
            
            if comm.rank == 0:
                np.savez(directory + "/data/debug_gmeans_" + str(cnt) + "_rank" + str(comm.rank + self.rank_start) + "_" + str(color), centroid=centroid, label=label, eigval=eigval, eigvec=eigvec,
    ...
    def generate_energy_distribution_data(self, Nruns_per_energy):
        ...
        for nd in range(data_size[comm.rank]):
            ...
            for nr in range(Nruns_per_energy):
                if wvl_dict[nr].size > 10:
                    RGB_list = self.RGB_readings2(wvl_dict[nr])
...
#                        RGB_list = RGB_list_raw[np.sum(RGB_list_raw>0.1, axis=1)==1,:]
                        ...
                        ind_d_mean, d_mean, RGB_weights = self.data_interpretation(xy_list, RGB_list) #, np.sum(RGB_list, axis=1)/np.sum(RGB_list, axis=1))
                        for i in range(3):
                            energy_distribution_per_proc[i,int(ind_d_mean[i]),int(data_disp[comm.rank]+nd)] += RGB_weights[i]
        ...
        np.savez(directory + "/data/energy_distribution_" + str(comm.rank + self.rank_start), energy_distribution=energy_distribution_per_proc, #radius_vs_Nphoton=radius_vs_Nphoton_per_proc,
    ...
    def generate_test_image(self, Nphoton_rate, Nimages, imed_sigma, imaging_mode=False):
        if not imaging_mode:
            if self.verbose:
                ...
                with open(directory + "/logs/Rank" + str(comm.rank + self.rank_start) + ".txt", 'w') as log:
...
#                    break
            ...
            RGB_list = self.RGB_readings2(wvl_list)
            ...
            if wvl_list.size > 0:
                t1 = time.time()
                if self.RGB_scintillator:
                    image_reconstr_temp, sc_ground_truth_temp, sc_temp, fm_temp, nCluster_temp, nPhotonR_temp, nPhotonG_temp, nPhotonB_temp, Mconf_temp = self.reconstruct_image(source_energy_list,
                                                                                                                                                                                 source_xy_list,
                                                                                                                                                                                 xy_list,
                                                                                                                                                                                 RGB_list,
                                                                                                                                                                                 event_label,
                    ...
                    image_reconstr_temp, sc_ground_truth_temp, sc_temp, fm_temp, nCluster_temp, nPhoton_temp, Mconf_temp = self.reconstruct_image(source_energy_list,
                                                                                                                                                  source_xy_list,
                                                                                                                                                  xy_list,
                                                                                                                                                  RGB_list,
                                                                                                                                                  event_label,
        ...
        if imaging_mode:
            ...
            if not os.path.exists(directory + "/results/image_reconstruction/" + self.identifier + "/Mconf_pRate" + str(Nphoton_rate)):
                os.mkdir(directory + "/results/image_reconstruction/" + self.identifier + "/Mconf_pRate" + str(Nphoton_rate))
            if comm.rank != 0:
                np.savez(directory + "/results/image_reconstruction/" + self.identifier + "/Mconf_pRate" + str(Nphoton_rate) + "/rank" + str(comm.rank + self.rank_start),
                         Mconf_per_proc=Mconf_per_proc)
                if self.RGB_scintillator:
                    np.savez(directory + "/results/image_reconstruction/" + self.identifier + "/nPhoton_pRate" + str(Nphoton_rate) + "/rank" + str(comm.rank + self.rank_start),
            ...
            if comm.rank == 0:
                for nr in range(1+self.rank_start, comm.size+self.rank_start):
                    ...
                    while True:
                        if os.path.exists(directory + "/results/image_reconstruction/" + self.identifier + "/Mconf_pRate" + str(Nphoton_rate) + "/rank" + str(nr) + ".npz"):
                            try:
                                with np.load(directory + "/results/image_reconstruction/" + self.identifier + "/Mconf_pRate" + str(Nphoton_rate) + "/rank" + str(nr) + ".npz") as data:
            ...
            if comm.rank == 0:
                if self.RGB_scintillator:
                    np.savez(directory + "/results/image_reconstruction/" + self.identifier + "/metrics_pRate_" + str(Nphoton_rate), max_imed=max_imed,
                             ...
                             sc_ground_truth=sc_ground_truth, Mconf=Mconf)
                else:
                    np.savez(directory + "/results/image_reconstruction/" + self.identifier + "/metrics_pRate_" + str(Nphoton_rate), max_imed=max_imed,
    ...
    def compute_imed(self, image_true, image_reconstr, sigma, Nphoton_rate): # image Euclidean distance
        ...
        image_true = image_true[:,:,self.energy_tgt_index_imed]
        image_reconstr = image_reconstr[:,:,self.energy_tgt_index_imed]
        
        filtered_true = np.zeros_like(image_true)
        filtered_reconstr = np.zeros_like(image_reconstr)
        
        for s in range(sigma.size):
            for n in range(self.energy_tgt_index_imed.size):
                filtered_true[:,:,n] = gaussian_filter(image_true[:,:,n], sigma[s], mode='reflect')
                filtered_reconstr[:,:,n] = gaussian_filter(image_reconstr[:,:,n], sigma[s], mode='reflect')
                
            max_true = np.max(filtered_true)
            min_true = np.min(filtered_true)
            max_reconstr = np.max(filtered_reconstr)
            min_reconstr = np.min(filtered_reconstr)
            
            if max_true - min_true > 0:
                norm_true = (filtered_true - min_true)/(max_true - min_true)
            else:
                norm_true = filtered_true.copy()
            if max_reconstr - min_reconstr > 0:
                norm_reconstr = (filtered_reconstr - min_reconstr)/(max_reconstr - min_reconstr)
            else:
                norm_reconstr = filtered_reconstr.copy()
                                                
            for n in range(self.energy_tgt_index_imed.size):
                dist[s,n] = np.sqrt(np.sum((norm_true[:,:,n] - norm_reconstr[:,:,n])**2))
            
                if comm.rank == 0:
                    if np.sum(image_true[:,:,n]) > 0:
                        np.savez(directory + "/data/reconstruction_examples/" + self.identifier + "/imed_pRate_" + str(Nphoton_rate) + "_n" + str(n) + "_sigma" + str(sigma[s]),
                                 filtered_true=filtered_true, filtered_reconstr=filtered_reconstr, dist=dist, image_true=image_true[:,:,n], image_reconstr=image_reconstr[:,:,n],
                                 norm_true=norm_true[:,:,n], norm_reconstr=norm_reconstr[:,:,n])
...
#                         dist=dist, image_true=image_true[:,:,n], image_reconstr=image_reconstr[:,:,n], slice_true=slice_true, slice_reconstr=slice_reconstr)
    ...
    def plot_trends(self, Nphoton_rate_list, Nimages, imed_sigma):
        ...
        max_imed = np.zeros((imed_sigma.size, Nrate))
        max_imed_range = np.zeros((imed_sigma.size, Nrate, 2))
#        sc_ground_truth = np.zeros(Nrate)
#        sc_ground_truth_range = np.zeros(2)
        silhouette_coeff = np.zeros(Nrate)
        silhouette_coeff_range = np.zeros((Nrate, 2))
        ...
        for nph in range(Nrate):
            if os.path.exists(directory + "/results/image_reconstruction/" + self.identifier + "/metrics_pRate_" + str(Nphoton_rate_list[nph]) + ".npz"):
                ...
                max_imed[:,nph] = np.mean(data['max_imed'], axis=1)
                max_imed_range[:,nph,0] = np.min(data['max_imed'], axis=1)
                max_imed_range[:,nph,1] = np.max(data['max_imed'], axis=1)
                #sc_ground_truth[nph] = data['sc_ground_truth']
                silhouette_coeff[nph] = np.mean(data['silhouette_coeff'])
                silhouette_coeff_range[nph,0] = np.min(data['silhouette_coeff'])
                silhouette_coeff_range[nph,1] = np.max(data['silhouette_coeff'])
                fowlkes_mallows_index[nph] = np.mean(data['fowlkes_mallows_index'])
                fowlkes_mallows_index_range[nph,0] = np.min(data['fowlkes_mallows_index'])
                fowlkes_mallows_index_range[nph,1] = np.max(data['fowlkes_mallows_index'])
                nCluster[nph] = data['nCluster']
                if Nphoton_rate_list[nph] == 1:
                    Mconf = data['Mconf']
                if self.RGB_scintillator:
        ...
        for ns in range(imed_sigma.size):
            lower_err = max_imed_range[ns,:,0] #np.min(max_imed[ns,:,:], axis=0)
            upper_err = max_imed_range[ns,:,1] #np.max(max_imed[ns,:,:], axis=0)
            mean_val = max_imed[ns,:] #np.mean(max_imed[ns,:,:], axis=0)
        ...
        lower_err = silhouette_coeff_range[:,0] #np.min(silhouette_coeff, axis=0)
        upper_err = silhouette_coeff_range[:,1] #np.max(silhouette_coeff, axis=0)
        mean_val = silhouette_coeff.copy() #np.mean(silhouette_coeff, axis=0)
        errorbar = [mean_val-lower_err,upper_err-mean_val]
        ...
        lower_err = fowlkes_mallows_index_range[:,0] #np.min(fowlkes_mallows_index, axis=0)
        upper_err = fowlkes_mallows_index_range[:,1] #np.max(fowlkes_mallows_index, axis=0)
        mean_val = fowlkes_mallows_index.copy() #np.mean(fowlkes_mallows_index, axis=0)
        errorbar = [mean_val-lower_err,upper_err-mean_val]
        ...
        plt.savefig(directory + "/plots/" + self.identifier + "_Confusion_Matrix.png", dpi=300)
        plt.close()
...
#        plt.close()
        ...
        np.savez(directory + "/results/" + self.identifier + "_metric_trend_data", max_imed=max_imed, silhouette_coeff=silhouette_coeff, fowlkes_mallows_index=fowlkes_mallows_index,
                 nCluster=nCluster, Mconf=Mconf)#, sc_ground_truth=sc_ground_truth)
        ...
        for nph in range(Nrate):
            if os.path.exists(directory + "/results/image_reconstruction/" + self.identifier + "/arrays_pRate" + str(Nphoton_rate_list[nph]) + ".npz"):
                with np.load(directory + "/results/image_reconstruction/" + self.identifier + "/arrays_pRate" + str(Nphoton_rate_list[nph]) + ".npz") as data:
                    image_reconstr_norm = data['image_reconstr5']
            else:
                image_reconstr_norm = np.zeros((self.NDetVoxel[0], self.NDetVoxel[1], self.energy_tgt.size-1))
                
            fig, ax = plt.subplots(2, 3, figsize=[30,20], dpi=300)

            vmin_true = np.min(image_true)
            vmax_true = np.max(image_true)
            vmin_reconstr = np.min(image_reconstr_norm)
            vmax_reconstr = np.max(image_reconstr_norm)
            ax[0,0].imshow(np.rot90(image_true[:,:,0]), cmap='gray', vmin=vmin_true, vmax=vmax_true, aspect=self.dimSampleVoxel[1]/self.dimSampleVoxel[0])
            ax[1,0].imshow(np.rot90(image_reconstr_norm[:,:,0]), cmap='gray', vmin=vmin_reconstr, vmax=vmax_reconstr)
            
            vmin_reconstr = np.min(image_reconstr_norm)
            vmax_reconstr = np.max(image_reconstr_norm)
            ax[0,1].imshow(np.rot90(image_true[:,:,1]), cmap='gray', vmin=vmin_true, vmax=vmax_true, aspect=self.dimSampleVoxel[1]/self.dimSampleVoxel[0])
            ax[1,1].imshow(np.rot90(image_reconstr_norm[:,:,1]), cmap='gray', vmin=vmin_reconstr, vmax=vmax_reconstr)
            
            vmin_true = np.min(image_true[:,:,1] - image_true[:,:,0])
            vmax_true = np.max(image_true[:,:,1] - image_true[:,:,0])
            vabs_true = np.max((np.abs(vmin_true), np.abs(vmax_true)))
            vmin_reconstr = np.min(image_reconstr_norm[:,:,1] - image_reconstr_norm[:,:,0])
            vmax_reconstr = np.max(image_reconstr_norm[:,:,1] - image_reconstr_norm[:,:,0])
            vabs_reconstr = np.max((np.abs(vmin_reconstr), np.abs(vmax_reconstr)))
            ax[0,2].imshow(np.rot90(image_true[:,:,1] - image_true[:,:,0]), cmap='coolwarm', vmin=-vabs_true, vmax=vabs_true, aspect=self.dimSampleVoxel[1]/self.dimSampleVoxel[0])
            ax[1,2].imshow(np.rot90(image_reconstr_norm[:,:,1] - image_reconstr_norm[:,:,0]), cmap='coolwarm', vmin=-vabs_reconstr, vmax=vabs_reconstr)
            ...
            ax[1,0].imshow(np.rot90(image_reconstr_norm[:,:,1]), cmap='gray', vmin=vmin_reconstr, vmax=vmax_reconstr)
            ...
            vabs_true = np.max((np.abs(vmin_true), np.abs(vmax_true)))
            vmin_reconstr = np.min(image_reconstr_norm[:,:,2] - image_reconstr_norm[:,:,1])
            vmax_reconstr = np.max(image_reconstr_norm[:,:,2] - image_reconstr_norm[:,:,1])
            vabs_reconstr = np.max((np.abs(vmin_reconstr), np.abs(vmax_reconstr)))
            ...
            ax[1,2].imshow(np.rot90(image_reconstr_norm[:,:,2] - image_reconstr_norm[:,:,1]), cmap='coolwarm', vmin=-vabs_reconstr, vmax=vabs_reconstr)
            ...
            ax[1,1].imshow(np.rot90(image_reconstr_norm[:,:,2]), cmap='gray', vmin=vmin_reconstr, vmax=vmax_reconstr)
...
if __name__ == '__main__':
    ...
    if RGB:
        ...
        Nlayers = 3
        scintillator_material = np.array([2,4,5])
        scintillator_thickness = np.array([0.0538629,0.10233,0.304055]) #np.array([0.050783,0.012911,0.037092])
        distribution_datafile = 'RGB_67keV_v2'
        identifier = 'RGB_67keV_v2'
        rank_start = 300
    else:
        RGB_scintillator = False
        Nlayers = 1
        scintillator_material = np.array([4])
        scintillator_thickness = np.array([0.0538629+0.10233+0.304055]) #np.array([0.050783+0.012911+0.037092])
        distribution_datafile = 'CsI_67keV_v2'
        identifier = 'CsI_67keV_v2_try2'
        rank_start = 0
    ...
    simulation = geant4(G4directory = "/home/gridsan/smin/geant4/G4_Nanophotonic_Scintillator-main",
                        ...
                        sampleID = 1, # 0: None, 1:AM, 2:AF #----------------------------------------------- Change
                        dimSampleVoxel = np.array([2.137,8,2.137])/np.array([400,400,2]), # in cm
                        ...
                        scintillator_material = scintillator_material, # 1:YAGCe, 2:ZnSeTe, 3:LYSOCe, 4:CsITl, 5:GSOCe #- Change
                        ...
                        energy_tgt_index_imed = np.array([0,1,2]),
                        energy_bins = 52,
                        d_mean_bins = 100,
...
#        print(i+1, flush=True)
    ...
    imaging_mode = False
    Nphoton_rate_list = np.hstack((np.ones(1), np.linspace(10, 100, 10))).astype(int)
    #np.hstack((np.linspace(10, 100, 10), np.linspace(200, 500, 4))).astype(int) / np.linspace(10, 100, 10).astype(int)
    Nphoton_rate_list_plot = np.hstack((np.ones(1), np.linspace(10, 100, 10))).astype(int)
    #np.hstack((np.linspace(10, 100, 10), np.linspace(200, 500, 4))).astype(int) / np.linspace(10, 100, 10).astype(int)
    imed_sigma = np.array([5,10,20,30,40,50])
    Nphoton = 1e3 #int(1e2) # originally 1e3
...
#    imaging_mode = True
#    Nphoton_rate_list = np.linspace(10, 100, 1).astype(int)
#    Nphoton_rate_list_plot = np.linspace(10, 100, 1).astype(int)
#    imed_sigma = np.array([5,10,20,30,40,50])
...
##        if comm.rank == 0:
##            with np.load(directory + "/results/image_reconstruction/" + simulation.identifier + "/arrays_pRate" + str(Nphoton_rate) + ".npz") as data: # 1e7 done
##                simulation.image_true = data['image_true']
##                simulation.image_reconstr = data['image_reconstr']
#        for i in range(100):
#            t1 = time.time()
#            if verbose:
#                print('### Run ' + str(i+1) + ' of 100', flush=True)
#            if comm.rank == 0:
