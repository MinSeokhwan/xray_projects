...
class geant4:
    def __init__(self,
        ...
        self.Tfilter = txt.read_txt(directory + "/data/rgb_filter")
        self.angle_T = np.linspace(0, 80, 9)*np.pi/180
        self.wvl_T = np.linspace(300, 1000, 701)
...
#            mac.write("/run/beamOn " + str(int(Nruns)))
    ...
    def read_output(self, Nruns):
        ...
        angle_list = np.arctan(pxy_list/pz_list)
        angle_ind = np.argmin(np.abs(self.angle_T[np.newaxis,:] - angle_list[:,np.newaxis]), axis=1)
        wvl_ind = np.argmin(np.abs(self.wvl_T[np.newaxis,:] - wvl_list[:,np.newaxis]), axis=1)
        T_rand = np.random.rand(wvl_list.size)
        T_mask = np.zeros(wvl_list.size).astype(bool)
        for i in range(wvl_list.size):
            if T_rand[i] < self.Tfilter[wvl_ind[i],angle_ind[i]+1]:
                T_mask[i] = True
            else:
                T_mask[i] = False
...
#        wvl_list = wvl_list[T_mask]
#        xy_list = xy_list[T_mask,:]
#        eventHits = eventHits[T_mask]
        ...
        np.savez(directory + "/data/debug_read_output" + str(comm.rank + self.rank_start), wvl_filt=wvl_list, eventHits=eventHits, xy_list=xy_list, energySrc_list=energySrc_list, xySrc_list=xySrc_list,
                 ...
                 fEventHits=np.array(hits["fEvent"].array()), fEventSrc=np.array(source["fEvent"].array()), wvl_list=np.array(photons["fWlen"].array())[scint_photon_ind], Tfilter=self.Tfilter,
                 T_mask=T_mask)
...
#        npts_vs_r = np.zeros(self.d_mean_bins)
#        r_list = np.sqrt(np.sum((np.tile(centroid[0,:], (npts,1)) - xy_list)**2, axis=1))
#        for i in range(self.d_mean_bins):
#            npts_vs_r[i] = np.sum(r_list < self.d_mean_list[i])
#            if npts_vs_r[i] > 0.9*npts:
#                d_mean = self.d_mean_list[i-1]
#                ind_d_mean = i - 1
#                break
...
#                     centroid=centroid, label=label, d_mean=d_mean, ind_d_mean=ind_d_mean, d_mean_list=self.d_mean_list)
    ...
    def reconstruct_image(self, source_energy_list, source_xy_list, xy_list, RGB_list, event_label, nCluster=None, nTrials=10):
        ...
        if self.RGB_scintillator:
            ...
            RGB_proj = (RGB_list @ np.array([-1,0,1]).reshape(-1, 1)).flatten()/np.sum(np.array([-1,0,1])**2)
            RGB_proj = RGB_proj.reshape(-1, 1)
            ...
            min_cluster_size = 10 # 100,200,200
            ...
            if xy_list.shape[0] > min_cluster_size:
                ...
                centroid, label = self.gmeans(np.hstack((xy_list, 10*RGB_proj)), 20, 20, min_cluster_size=min_cluster_size, max_cluster_size=max_cluster_size, RGB=True)
                nk = centroid.shape[0]
                ind_RGB = np.argmax(np.abs(centroid[:,2][:,np.newaxis] - 10*np.array([1/3,0,-1/3])[np.newaxis,:]), axis=1)
                        
                sc = silhouette_coeff(nk, xy_list, np.ones(xy_list.shape[0]), centroid=centroid[:,:2], label=label)
...
#                        r_list = np.sqrt(np.sum((np.tile(centroid[k,:2], (npts,1)) - xy_temp)**2, axis=1))
#                        for i in range(self.d_mean_bins):
...
#                                break
                        ...
                        d_mean[k] = np.sum(np.sqrt(np.sum((np.tile(centroid[k,:2], (npts,1)) - xy_temp[kde>np.max(np.hstack((kde, kde_temp)))/10,:])**2, axis=1)))/npts
                        ind_d_mean = np.argmin(np.abs(d_mean[k] - self.d_mean_list))
                        
                        while True:
                            xrayInc = energy_distribution_norm[ind_RGB[k],ind_d_mean,:]
                        ...
                        ratio = np.zeros(4)
                        ratio[0] = np.sqrt(np.sum((np.array([x0,y0]) - centroid[k,:2])**2))
                        ratio[1] = np.sqrt(np.sum((np.array([x0,y1]) - centroid[k,:2])**2))
                        ratio[2] = np.sqrt(np.sum((np.array([x1,y0]) - centroid[k,:2])**2))
                        ratio[3] = np.sqrt(np.sum((np.array([x1,y1]) - centroid[k,:2])**2))
                        if np.any(ratio == 0):
                ...
                if fm < 0.5:
                    if os.path.exists(directory + "/data/reconstruction_examples/" + self.identifier + "/clustering_pRate_" + str(nk) + "_min.npz"):
                        ...
                        if fm < data['fm']:
                            np.savez(directory + "/data/reconstruction_examples/" + self.identifier + "/clustering_pRate_" + str(nk) + "_min", xrayInc=xrayInc_all,
                                     ind_x0=ind_x0, ind_x1=ind_x1, x0=x0, x1=x1, ind_y0=ind_y0, ind_y1=ind_y1, y0=y0, y1=y1, ratio=ratio, xy_list=xy_list, RGB_list=RGB_list,
                                     source_energy_list=source_energy_list, source_xy_list=source_xy_list, event_label=event_label, d_mean=d_mean,
                                     sc=sc, ind_RGB=ind_RGB, fm=fm, energy_distribution_norm=energy_distribution_norm, RGB_proj=RGB_proj,
                                     sc_ground_truth=sc_ground_truth, centroid=centroid, label=label)
                    else:
                        np.savez(directory + "/data/reconstruction_examples/" + self.identifier + "/clustering_pRate_" + str(nk) + "_min", xrayInc=xrayInc_all,
                                     ind_x0=ind_x0, ind_x1=ind_x1, x0=x0, x1=x1, ind_y0=ind_y0, ind_y1=ind_y1, y0=y0, y1=y1, ratio=ratio, xy_list=xy_list, RGB_list=RGB_list,
                                     source_energy_list=source_energy_list, source_xy_list=source_xy_list, event_label=event_label, d_mean=d_mean,
                    ...
                    if os.path.exists(directory + "/data/reconstruction_examples/" + self.identifier + "/clustering_pRate_" + str(nk) + "_max.npz"):
                        ...
                        if fm > data['fm']:
                            np.savez(directory + "/data/reconstruction_examples/" + self.identifier + "/clustering_pRate_" + str(nk) + "_max", xrayInc=xrayInc_all,
                                     ind_x0=ind_x0, ind_x1=ind_x1, x0=x0, x1=x1, ind_y0=ind_y0, ind_y1=ind_y1, y0=y0, y1=y1, ratio=ratio, xy_list=xy_list, RGB_list=RGB_list,
                                     source_energy_list=source_energy_list, source_xy_list=source_xy_list, event_label=event_label, d_mean=d_mean,
                                     sc=sc, ind_RGB=ind_RGB, fm=fm, energy_distribution_norm=energy_distribution_norm, RGB_proj=RGB_proj,
                                     sc_ground_truth=sc_ground_truth, centroid=centroid, label=label)
            ...
            if xy_list.shape[0] > 200:
                sc_ground_truth = silhouette_coeff(int(np.max(event_label)), xy_list, np.ones(xy_list.shape[0]), centroid=source_xy_list, label=event_label)
            
                centroid, label = self.gmeans(xy_list, 20, 20, min_cluster_size=200, max_cluster_size=2500, RGB=False)
                nk = centroid.shape[0]
            ...
            for nC in range(centroid_prev.shape[0]):
                ...
                if xy_nC.shape[0] > min_cluster_size:
                    try:
                        ...
                        np.savez(directory + "/data/debug_pca", xy_nC=xy_nC)
                        eigval, eigvec = pca(xy_nC)
...
#                        if pval == 0:
#                            np.savez(directory + "/data/debug_diptest", xy_nC=xy_nC, xy_proj=xy_proj, statistic=statistic, eigvec_all=eigvec_all, nC=nC, centroid=centroid_temp[2*nC:2*(nC+1),:],
#                                     centroid_prev=centroid_prev[nC,:], label=label[mask_nC])
                    else:
                        if centroid is None:
...
#                        log.write('Cluster ' + str(nC) + ': ' + str(pval) + ' / ' + str(np.sum(label_temp==2*nC)) + ' / ' + str(np.sum(label_temp==2*nC+1)) + '\n')
            ...
            if comm.rank == 0:
                np.savez(directory + "/data/debug_gmeans_pre_merge_" + str(cnt) + "_rank" + str(comm.rank + self.rank_start), centroid=centroid, label=label, eigval=eigval, eigvec=eigvec,
                         ...
                         label_temp=label_temp, xy_nC=xy_nC, label_nC=label_nC, split_check=split_check, cnt=cnt)
            ...
            if centroid.shape[0] > 1:
                ...
                for nC1 in range(centroid.shape[0]):
                    if not ind_merged[nC1] and np.sum(label==nC1) > 0:
                        dist = np.sqrt(np.sum((centroid[nC1,:2][np.newaxis,:] - centroid[:,:2])**2, axis=1))
                        ...
                        if (pval > p_crit and np.sum(mask_nC) < max_cluster_size):
                            ...
                            if centroid_merged is None:
                                centroid_merged = np.mean(xy_list[mask_nC,:], axis=0)
                                label_merged[mask_nC] = 0
                            else:
                                centroid_merged = np.vstack((centroid_merged, np.mean(xy_list[mask_nC,:], axis=0)))
            ...
            if comm.rank == 0:
                np.savez(directory + "/data/debug_gmeans_" + str(cnt) + "_rank" + str(comm.rank + self.rank_start), centroid=centroid, label=label, eigval=eigval, eigvec=eigvec,
                         centroid_temp=centroid_temp, centroid_prev=centroid_prev, xy_proj=xy_proj, pval=pval_save, label_prev=label_prev, ind_label=ind_label, xy_list=xy_list,
                         label_temp=label_temp, xy_nC=xy_nC, label_nC=label_nC, split_check=split_check, cnt=cnt, fm=fm)
                
            cnt += 1
            
            if cnt > maxiter_gmeans or fm == 1: #not split_check or 
                break
...
#                if ind_merged[nC] == 0:
#                    dist = np.sqrt(np.sum((np.tile(centroid[nC,:2], (centroid.shape[0],1)) - centroid[:,:2])**2, axis=1))
#                    if np.sum(dist < self.dimDet[0]/2.5e4) > 1:
#                        index = np.argwhere(dist < self.dimDet[0]/2.5e4)[:,0]
#                        
#                        mask_nC = np.isin(label, index)
#                        
#                        for i in index:
#                            ind_merged[i] = 1
#                        
...
#                    else:
#                        if centroid_merged is None:
#                            centroid_merged = centroid[nC,:]
#                            label_merged[label==nC] = 0
#                        else:
#                            centroid_merged = np.vstack((centroid_merged, centroid[nC,:]))
#                            label_merged[label==nC] = np.max(label_merged) + 1
#        else:
...
#            np.savez(directory + "/data/debug_gmeans_" + str(cnt) + "_rank" + str(comm.rank + self.rank_start), centroid=centroid, label=label, eigval=eigval, eigvec=eigvec,
#                     centroid_temp=centroid_temp, centroid_prev=centroid_prev, xy_proj=xy_proj, pval=pval_save, label_prev=label_prev, ind_label=ind_label, xy_list=xy_list,
#                     label_temp=label_temp, xy_nC=xy_nC, label_nC=label_nC, split_check=split_check, cnt=cnt, fm=fm, centroid_merged=centroid_merged, label_merged=label_merged)
#        
#        return centroid_merged, label_merged
...
#                    break
            ...
            if wvl_list.size > 0:
                ...
                if not imaging_mode:
                    ...
                    nPhoton_per_proc = np.hstack((nPhoton_per_proc, nPhoton_temp))
            else:
                image_reconstr_temp = np.zeros_like(self.image_reconstr).astype(np.float32)
    ...
    def compute_imed(self, image_true, image_reconstr, sigma, Nphoton_rate): # image Euclidean distance
        ...
        for s in range(sigma.size):
            for n in range(self.energy_tgt_index_imed.size):
                filtered_true = gaussian_filter(norm_true[:,:,n], sigma[s], mode='reflect')
                filtered_reconstr = gaussian_filter(norm_reconstr[:,:,n], sigma[s], mode='reflect')
...
#        print(i+1, flush=True)
    ...
    imaging_mode = False
    Nphoton_rate_list = np.hstack((np.linspace(30, 100, 10), np.linspace(200, 500, 4))).astype(int)
    #np.hstack((np.linspace(10, 100, 10), np.linspace(200, 500, 4))).astype(int) / np.linspace(10, 100, 10).astype(int)
    Nphoton_rate_list_plot = np.hstack((np.linspace(30, 100, 10), np.linspace(200, 500, 4))).astype(int)
    #np.hstack((np.linspace(10, 100, 10), np.linspace(200, 500, 4))).astype(int) / np.linspace(10, 100, 10).astype(int)
    imed_sigma = np.array([5,10,20,30,40,50])
    Nimages = int(1e2) # originally 1e3
